...
const Round3: React.FC = () => {
    ...
    const unsubscribe = onSnapshot(gameRef, (snapshot) => {
      ...
      if (data) {
        ...
        if (data.gameStatus === 'results') {
          navigate(`/results/${pin}`, { state: { playerKey } });
        }
      }
    });
  ...
  const handleExchange = async (exchanged: boolean) => {
    ...
    if (exchanged && playerData) {
      ...
      const updatedPlayerData = {
        ...playerData,
        Room: [playerData.Room[0], playerData.Room[1], playerData.Room[2], newRoom],
      };
  ...
  const seeResults = async () => {
    ...
    navigate(`/results/${pin}`, { state: { playerKey, playerData } });
  };

  const playersInSameRoom = gameData?.players
    ? Object.values(gameData.players).filter(
        (player) => player.Room[2] === playerData?.Room[2]
      )
    : [];
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-blue-100 text-center">
      {showExchange ? (
        ...
        <div>
          <span className="text-2xl font-bold text-gray-800">ROUND 3</span>
          <Countdown onComplete={handleCountdownComplete} duration={10} />
          <RoomInfo
            roomNumber={playerData?.Room?.[2]}
